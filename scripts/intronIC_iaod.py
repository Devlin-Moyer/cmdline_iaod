...
def add_tags(
    ...
    # for attr in dir(intron):  ###!!!
    #     print("obj.%s = %r" % (attr, getattr(intron, attr)))###!!!

    # print(intron.get_name(), intron.longest_isoform)
        ...
        for intron in ints:
            ...
            for matrix in matrices:
                m_score, *_, seq = bp_score(
                    bp_region_seq, matrix, use_bpx=True)
...
def add_scores(introns, intron_seq_file):
    ...
    iaod_file = open('info/{}_info.iic'.format(SPECIES), 'w')

    tmp_file = 'tmp/{}.tmp'.format(intron_seq_file)
    score_dict = {
    ...
    with open(intron_seq_file) as inf, open(tmp_file, 'w') as outf:
        for l in inf:
            ...
            try:
                iaod_bits = iaod_dict[name]
                seq = bits[4]
                iaod_bits.append(seq)
                iaod_file.write('\t'.join([str(e) for e in iaod_bits]) + '\n')
            except KeyError:
                pass
...
parser.add_argument(
    '--debug',
    action='store_true',
    help='Produce additional scoring information for debugging'
)
parser.add_argument(
    '-C',
    '--hyperparameter_C',
...
OPTIMIZE_N = 5
INTRON_FLANK_SIZE = 200  # for sending to file
MIN_INTRON_LENGTH = args.min_intron_len
...
if not SEQUENCE_INPUT:
    ...
    for intron in get_sub_seqs(
        ...
        # print(intron.get_name(), intron.omitted) ###!!!
        ...
        # add tags to introns with filtering
        # if not intron.omitted or intron.omitted == 's':
        #     intron, intron_index, longest_isoforms = add_tags(
        #         intron, intron_index, longest_isoforms,
        #         ALLOW_OVERLAP, LONGEST_ONLY)
        ...
        # write omitted introns to list file (unscored)
            ...
            if not ONLY_SEQS:
                meta_bits = [
                    ...
                    intron.md5.hex(),
                    'fractional_position',
                    '.', # placeholder for type_id
                    '.'  # placeholder for label_ratio
                ]
...
for index, intron in enumerate(sorted(
    ...
    iaod_bits = [
        ...
        intron.relative_score,  # >0 = U12 by default
        intron.region,  # genomic header
        intron.start,
        intron.stop,
        intron.strand,
        intron.phase,
        intron.index, # intron rank within transcript
    ...
    iaod_dict[intron.get_name().split(';')[0]] = iaod_bits
...
if DEBUG:
    debug_info_fn = '{}.debug.iic'.format(SPECIES)
    debug_file = open(debug_info_fn, 'w')
    scored_seqs = ['five_seq', 'bp_seq', 'three_seq']
    region_tags = ['five', 'bp', 'three']
    starts = [FIVE_SCORE_COORDS[0], 0, THREE_SCORE_COORDS[0]]
    ...
    for index, intron in enumerate(
        ...
        if getattr(intron, 'demote_info', None) is not None:
            ...
            u2_key = ('u2', 'gtag')
        score_strings = []
        header = '\t'.join([intron.get_name(),str(intron.svm_score)])
        debug_file.write('>{}\n'.format(header))
        ...
        score_strings.append(('-'.join(u12_key), '', '-'.join(u2_key)))
        for line_bits in zip(*score_strings):
            debug_file.write('\t'.join(line_bits) + '\n')
    debug_file.close()
